...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `siap`.`vta_matriculados_por_grupo` (`codigo_taller` INT, `nivel_taller` INT, `descripcion` INT, `id_grupo` INT, `periodo` INT, `periodo_final` INT, `dia` INT, `hora` INT, `hora_final` INT, `nombre_profesor` INT, `id_matricula` INT, `activa` INT, `created_at` INT, `id_estudiante` INT, `id_usuario` INT, `cedula` INT, `apellido` INT, `nombre` INT, `foto` INT, `correo` INT);
...
update t_grupo set cupo_actual = vpi_cupoActual;
