...
CREATE TABLE IF NOT EXISTS `siap`.`t_estudiante` (
  ...
  `padecimientos` VARCHAR(2000) CHARACTER SET 'latin1' NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `siap`.`t_taller` (
  ...
  `costo_funcionario` INT UNSIGNED NOT NULL,
...
CREATE TABLE IF NOT EXISTS `siap`.`t_grupo` (
  ...
  `profesor` INT NOT NULL,
  `taller` INT NOT NULL,
  `cupo_base` INT UNSIGNED NOT NULL,
  `cupo_extra` INT UNSIGNED NOT NULL,
  `cupo_actual` INT UNSIGNED NULL DEFAULT '0',
  `periodo` DATE NULL DEFAULT NULL,
  `periodo_final` DATE NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `siap`.`t_casillero` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `codigo` VARCHAR(45) NOT NULL,
  `tipo` VARCHAR(20) NOT NULL DEFAULT 'M' COMMENT 'M:mujeres, H: hombres',
...
CREATE TABLE IF NOT EXISTS `siap`.`t_matricula` (
  ...
  `qr` VARCHAR(200) NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `siap`.`t_padecimiento` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `descripcion` VARCHAR(2000) CHARACTER SET 'latin1' NOT NULL,
  `estudiante` INT NOT NULL,
  `observaciones` VARCHAR(2000) CHARACTER SET 'latin1' NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
...
-- -----------------------------------------------------
-- Table `siap`.`t_progreso`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `siap`.`t_progreso` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `estudiante` INT NOT NULL,
  `nombre_taller` VARCHAR(255) NOT NULL,
  `codigo_taller` VARCHAR(45) NOT NULL,
  `descripcion` VARCHAR(1000) NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_estudiante_idx` (`estudiante` ASC) VISIBLE,
  INDEX `fk_tallerdescripcion_progreso_idx` (`codigo_taller` ASC) VISIBLE,
  CONSTRAINT `fk_estudiante`
    FOREIGN KEY (`estudiante`)
    REFERENCES `siap`.`t_estudiante` (`id`)
    ON DELETE CASCADE
    ON UPDATE CASCADE,
  CONSTRAINT `fk_tallerdescripcion_progreso`
    FOREIGN KEY (`codigo_taller`)
    REFERENCES `siap`.`t_taller` (`codigo`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
DEFAULT CHARACTER SET = latin1;
...
CREATE TABLE IF NOT EXISTS `siap`.`t_reposicion` (
  ...
  `estado` INT NOT NULL DEFAULT '0' COMMENT '0 = no aceptada, 1 = aceptada',
  PRIMARY KEY (`id`),
...
-- Table `siap`.`t_urls`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `siap`.`t_urls` (
  `id` INT NOT NULL AUTO_INCREMENT,
...
CREATE TABLE IF NOT EXISTS `siap`.`t_usuario_temp` (
  ...
  `comprobante` VARCHAR(200) NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  UNIQUE INDEX `cedula_UNIQUE` (`cedula` ASC) VISIBLE,
  UNIQUE INDEX `correo_UNIQUE` (`correo` ASC) VISIBLE)
ENGINE = InnoDB
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `siap`.`vta_admin_estudiante` (`id` INT, `cedula` INT, `foto` INT, `apellido` INT, `nombre` INT, `nacimiento` INT, `usuario` INT, `sexo` INT, `correo` INT, `id_estudiante` INT, `celular` INT, `telefono` INT, `nivel` INT, `descripcion` INT, `carrera_departamento` INT, `cantidad_dias` INT, `telefono_emergencia` INT, `provincia` INT, `canton` INT, `distrito` INT, `direccion` INT, `moroso` INT, `estado` INT, `padecimientos` INT, `prematricula` INT, `motivo_ingreso` INT, `tipo` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `siap`.`vta_asistencia` (`id_asistencia` INT, `fecha` INT, `id_matricula` INT, `id_grupo` INT, `id_taller` INT, `descripcion` INT, `codigo` INT, `dia` INT, `hora` INT, `hora_final` INT, `id_estudiante` INT, `foto` INT, `id_usuario` INT, `cedula` INT, `apellido` INT, `nombre` INT, `estado` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `siap`.`vta_casilleros` (`id_casillero` INT, `codigo_casillero` INT, `tipo` INT, `id_reserva` INT, `id_estudiante` INT, `id_usuario` INT, `cedula` INT, `hora_entrada` INT, `hora_salida` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `siap`.`vta_grupos` (`codigo_taller` INT, `costo` INT, `costo_funcionario` INT, `nivel` INT, `descripcion` INT, `id_taller` INT, `color` INT, `id_grupo` INT, `cupo_base` INT, `cupo_extra` INT, `cupo_actual` INT, `periodo` INT, `periodo_final` INT, `id_horario` INT, `dia` INT, `hora` INT, `hora_final` INT, `apellido` INT, `nombre` INT, `id_profesor` INT, `cedula` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `siap`.`vta_matriculados_por_grupo` (`codigo_taller` INT, `nivel_taller` INT, `descripcion` INT, `id_grupo` INT, `periodo` INT, `periodo_final` INT, `dia` INT, `hora` INT, `hora_final` INT, `cedula_profeosr` INT, `nombre_profesor` INT, `id_matricula` INT, `qr` INT, `activa` INT, `created_at` INT, `id_estudiante` INT, `id_usuario` INT, `cedula` INT, `apellido` INT, `nombre` INT, `foto` INT, `correo` INT);
...
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `siap`.`vta_profesor_reposiciones` (`id_reposicion` INT, `id_profesor_origen` INT, `id_usuario_origen` INT, `cedula_origen` INT, `nombre_origen` INT, `apellido_origen` INT, `id_profesor_reposicion` INT, `id_usuario_reposicion` INT, `cedula_reposicion` INT, `nombre_reposicion` INT, `apellido_reposicion` INT, `fecha_reposicion` INT, `id_grupo` INT, `id_horario` INT, `dia` INT, `hora` INT, `hora_final` INT, `id_taller` INT, `codigo_taller` INT, `nivel` INT, `descripcion` INT, `cupo_base` INT, `cupo_extra` INT, `cupo_actual` INT, `periodo` INT);

-- -----------------------------------------------------
-- Placeholder table for view `siap`.`vta_profesores`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `siap`.`vta_profesores` (`id_usuario` INT, `cedula` INT, `apellido` INT, `nombre` INT, `nacimiento` INT, `usuario` INT, `sexo` INT, `clave` INT, `foto` INT, `correo` INT, `id_profesor` INT, `rol` INT, `aso` INT);

-- -----------------------------------------------------
-- Placeholder table for view `siap`.`vta_reposiciones`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `siap`.`vta_reposiciones` (`id_reposicion` INT, `grupo_reposicion` INT, `fecha_reposicion` INT, `dia_reposicion` INT, `hora_reposicion` INT, `hora_reposicion_final` INT, `comprobante` INT, `observacion` INT, `estado` INT, `descripcion` INT, `id_estudiante` INT, `nivel` INT, `id_usuario` INT, `cedula` INT, `nombre` INT, `apellido` INT);

-- -----------------------------------------------------
-- Placeholder table for view `siap`.`vta_usuario_temp`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `siap`.`vta_usuario_temp` (`id` INT, `cedula` INT, `foto` INT, `nombre` INT, `apellido` INT, `nacimiento` INT, `usuario` INT, `sexo` INT, `created_at` INT, `tipo_usuario` INT, `estado` INT, `correo` INT, `comprobante` INT);
...
DELIMITER $$
USE `siap`$$
CREATE DEFINER=`root`@`localhost` FUNCTION `fs_cedula_estudiante_por_id`(vpi_id int(11)) RETURNS varchar(45) CHARSET utf8mb3
    DETERMINISTIC
BEGIN
...
begin 
    ...
    carrera_departamento = vps_carrera_departamento, tipo = vps_perfil 
    where id = fi_id_estudiante_por_cedula(vps_cedula);
end$$
...
in vpi_periodo date,
in vpi_periodoFinal date
)
begin
	update t_grupo set horario = vpi_horario, profesor = vpi_profesor, taller = vpi_taller, cupo_base = vpi_cupobase, cupo_extra = vpi_cupoextra, periodo = vpi_periodo, periodo_final = vpi_periodoFinal   
    where id = vpi_id;
end$$
...
DELIMITER $$
USE `siap`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `prc_eliminar_matricula`(IN `vpi_id_matricula` INT, IN `vpi_cupoActual` INT, IN `vpi_grupo` INT)
begin
delete from
    t_matricula
where
...
update t_grupo set cupo_actual = vpi_cupoActual WHERE id = vpi_grupo;
...
begin
    ...
    SELECT usuario into v_id_profesor from t_profesor WHERE id = vpi_id;

	delete from t_profesor
    where id = vpi_id;

    delete from t_usuario where id = v_id_profesor;
end$$
...
DELIMITER $$
USE `siap`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `prc_insertar_casillero`(IN `vpi_codigo` VARCHAR(45), IN `vpi_tipo` VARCHAR(20))
begin
	insert into t_casillero(codigo,estado,tipo)
    values (vpi_codigo,0,vpi_tipo);
    commit;
end$$
...
DELIMITER $$
USE `siap`$$
CREATE DEFINER=`root`@`localhost` PROCEDURE `prc_insertar_usuario`(IN `vps_cedula` VARCHAR(45), IN `vps_nombre` VARCHAR(100), IN `vps_apellido` VARCHAR(100), IN `vpd_nacimiento` DATE, IN `vps_usuario` VARCHAR(100), IN `vps_clave` VARCHAR(45), IN `vps_sexo` VARCHAR(45), IN `vps_correo` VARCHAR(100))
begin
...
begin
	declare vli_id int;
	insert into `siap`.`t_usuario`(`cedula`, `nombre`, `apellido`, `nacimiento`, `usuario`, `clave`, `sexo`, `correo`)
    values (vps_cedula, UPPER(vps_nombre), UPPER(vps_apellido), vpd_nacimiento, vps_usuario, sha1('1234'), vps_sexo, vps_correo);
...
USE `siap`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `siap`.`vta_asistencia` AS select distinct `a`.`id` AS `id_asistencia`,date_format(`a`.`fecha`,'%Y-%m-%d') AS `fecha`,`m`.`id` AS `id_matricula`,`g`.`id` AS `id_grupo`,`t`.`id` AS `id_taller`,`t`.`descripcion` AS `descripcion`,`t`.`codigo` AS `codigo`,`h`.`dia` AS `dia`,`h`.`hora` AS `hora`,`h`.`hora_final` AS `hora_final`,`e`.`id` AS `id_estudiante`,`u`.`foto` AS `foto`,`u`.`id` AS `id_usuario`,`u`.`cedula` AS `cedula`,`u`.`apellido` AS `apellido`,`u`.`nombre` AS `nombre`,`a`.`estado` AS `estado` from ((((((`siap`.`t_asistencia` `a` join `siap`.`t_matricula` `m`) join `siap`.`t_grupo` `g`) join `siap`.`t_taller` `t`) join `siap`.`t_horario` `h`) join `siap`.`t_estudiante` `e`) join `siap`.`t_usuario` `u`) where ((`a`.`estudiante` = `e`.`id`) and (`a`.`grupo` = `g`.`id`) and (`g`.`horario` = `h`.`id`) and (`g`.`taller` = `t`.`id`) and (`m`.`estudiante` = `e`.`id`) and (`m`.`grupo` = `g`.`id`) and (`e`.`usuario` = `u`.`id`));
...
USE `siap`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `siap`.`vta_profesores` AS select `u`.`id` AS `id_usuario`,`u`.`cedula` AS `cedula`,`u`.`apellido` AS `apellido`,`u`.`nombre` AS `nombre`,`u`.`nacimiento` AS `nacimiento`,`u`.`usuario` AS `usuario`,`u`.`sexo` AS `sexo`,`u`.`clave` AS `clave`,`u`.`foto` AS `foto`,`u`.`correo` AS `correo`,`p`.`id` AS `id_profesor`,`p`.`rol` AS `rol`,`p`.`aso` AS `aso` from (`siap`.`t_usuario` `u` join `siap`.`t_profesor` `p`) where (`u`.`id` = `p`.`usuario`) order by `u`.`apellido`,`u`.`nombre`;
